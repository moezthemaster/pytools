Parfait ! Voici le code complet de l'outil copy avec toutes les améliorations.

---

1. src/tools/copy.py (version finale)
...
from src.tools.copy import main
main()
" "$@"
```
...
3. Mise à jour de install.sh

Ajouter ces lignes dans la section de création des wrappers :

```bash
# Création du script bin/copy
cat > "$PROJECT_DIR/bin/copy" << 'EOF'
#!/bin/bash
PROJECT_DIR="$HOME/.sessions"
source "$PROJECT_DIR/venv/bin/activate"
python3 -c "
...
chmod +x "$PROJECT_DIR/bin/copy"
ln -sf "$PROJECT_DIR/bin/copy" "$BIN_DIR/copy"
```

---

4. Mise à jour de src/tools/__init__.py

```python
#!/usr/bin/env python3
from src.tools.connect import ConnectTool
from src.tools.exec import ExecTool
from src.tools.copy import CopyTool

__all__ = ['ConnectTool', 'ExecTool', 'CopyTool']
```

---

5. Mise à jour de src/__init__.py
...
6. Mise à jour de src/main.py (si tu utilises un point d'entrée unifié)
...
def main():
    if len(sys.argv) < 2:
        ...
        print("  exec <app> [env|all|...] <commande> [options]  → Exécution de commandes")
        print("  copy <source> <dest> [options]        → Copie de fichiers/répertoires")
        print()
        print("Exemples :")
        print("  connect appli1 prod")
        print("  exec appli1 all 'df -h' -u root")
        print("  copy appli1:prod:/var/log/app.log ./logs/ -u root --mode 644")
        print("  copy ./mon_script.sh appli1:prod:/tmp/ -u devops --mode 755")
        sys.exit(1)
    ...
    if command == "connect":
        ...
        print("Commandes disponibles : connect, exec, copy")
        sys.exit(1)
...
```bash
# Remote → Local avec sudo et permissions
copy appli1:prod:/var/log/app.log ./logs/ -u root --mode 644

# Local → Remote avec changement d'utilisateur
copy ./mon_script.sh appli1:prod:/tmp/ -u devops --mode 755
...
# Mode verbeux
copy appli1:prod:/var/log/app.log ./logs/ -v -u root --mode 644
```
...
Remote → Local Via /tmp + scp (ou scp direct si pas de -u)
Local → Remote Via /tmp + scp (ou scp direct si pas de -u)
Remote → Remote Via local (2 étapes)
Répertoires Via tar + scp
Permissions --mode pour définir les droits
Propriétaire chown user:group avec groupe par défaut
Nettoyage try/finally garanti
Progression -v pour scp -v

---

C'est prêt ! Tu peux tester et me dire si tout fonctionne comme prévu.
