...
""")
    ...
    def _copy_remote_to_local_without_sudo(self, dest_path):
        ...
        try:
            if is_directory:
                ...
                # Télécharger le tar
                self._sftp_transfer(client, remote_temp, dest_path + '.tar.gz', direction="download")
                ...
                subprocess.call(f"tar xzf {dest_path}.tar.gz -C {os.path.dirname(dest_path)}", shell=True)
                os.remove(dest_path + '.tar.gz')
            else:
                # Fichier : SFTP direct
    ...
    def _copy_local_dir_to_remote(self):
        ...
        try:
            ...
            if self.target_user:
                ...
                cmd = f"tar xzf {remote_temp} -C {self.dst_path}"
            result = self._execute_ssh(self.dst_app, self.dst_env, cmd)
            if result['code'] != 0:
    ...
    def _copy_remote_to_remote(self):
        ...
        try:
            ...
            if self.target_user:
                # Avec sudo : copier vers /tmp sur le remote source puis télécharger
                remote_temp = f"/tmp/{temp_name}"
                ...
                if is_directory:
                    ...
                    client = self._get_ssh_client(self.src_app, self.src_env)
                    try:
                        self._sftp_transfer(client, remote_temp, local_temp + '.tar.gz', direction="download")
                    finally:
                        client.close()
                    
                    # Extraire le tar localement
                    os.makedirs(local_temp, exist_ok=True)
                    subprocess.call(f"tar xzf {local_temp}.tar.gz -C {os.path.dirname(local_temp)}", shell=True)
                    os.remove(local_temp + '.tar.gz')
                    ...
                    client = self._get_ssh_client(self.src_app, self.src_env)
                    try:
                        self._sftp_transfer(client, remote_temp, local_temp, direction="download")
                    finally:
                        client.close()
                
                # Nettoyer le remote_temp
                self._execute_ssh(self.src_app, self.src_env, f"rm -f {remote_temp}")
            ...
            original_target_user = self.target_user
            ...
            try:
                ...
                self.target_user = original_target_user
    ...
    def _copy_local_dir_to_remote_with_temp(self, local_path):
        ...
        try:
            # Créer un tar du répertoire local
            print("📦 Compression du répertoire local...")
            subprocess.call(f"tar czf {local_temp} -C {os.path.dirname(local_path)} {os.path.basename(local_path)}", shell=True)
